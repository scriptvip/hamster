...
def recursive_buy_upgrade(token: str, upgrade_id: str, upgrade_name: str, level: int, profitPerHour: float, price: float):
    
    res = buy_upgrade(token, upgrade_id, upgrade_name, level, profitPerHour, price)

    if res == 'not_available': # mening the card need another card
        __card_id, __target_level = needed_card_id, needed_target_level
        card = None
        upgrades = available_upgrades(token)
        for __card in upgrades:
            if __card['id'] == __card_id:
                card = __card
                break
        if not card:
            log(f"{mrh} error card not found please contact with abdo sleem in this error !!!", flush=True)
        
        # print(f"{card['id']} : FROM {int(card['level'])} => {int(__target_level)}")

        for __level in range(int(card['level']), int(__target_level)+1):
            res = recursive_buy_upgrade(
                token,
                card['id'],
                card['name'],
                card['level'],
        ...
        return buy_upgrade(token, upgrade_id, upgrade_name, level, profitPerHour, price)
...
def execute_combo(token: str):
    ...
    with open("upgraders.py", 'w', encoding='utf-8') as f:
        f.write(str(upgrades))
